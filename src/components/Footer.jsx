...
const Footer = () => {
    const items = [{head: "PLATFORM", text: ["Find Work", "Find Talent", "Categories", "About Us"]},
        {head: "CATEGORIES", text: ["Data Science", "IT & Networking", "Web & Mobile"]},
        {head: "HELP", text: ["FAQ’s", "Contact Us"]},
        {head: "GET IN TOUCH @", text: ["Instagram", "itemedIn", "Twitter"]},
        ]
  return (
            ...
            <div className='pt-52 pb-36 grid lg:grid-cols-3 lg:px-10 gap-x-10 lg:items-start items-center' data-aos="fade-right">
                <div className='flex flex-col items-center lg:items-start gap-10 w-screen'>
                    ...
                    <div className='flex flex-col gap-6 items-center lg:items-start pb-20 lg:pb-0'>
                        <p className='text-white opacity-[20%]'>items AND REDIRECTS</p>
                    ...
                    <div className='flex lg:flex-row flex-col items-center lg:justify-between lg:pr-10'>
                        {
                            items.map((item, index) =>(
                                <div key={index} className='flex flex-col gap-2 lg:gap-6 text-center lg:text-start pb-6 lg:pb-0'>
                                    <p className='text-[14px] font-[600] text-white opacity-[20%]'>{item.head}</p>
                                    <p className='text-[18px] text-white'>{item.text[0]}</p>
                                    <p className='text-[18px] text-white'>{item.text[1]}</p>
                                    <p className='text-[18px] text-white'  >{item.text[2]}</p>
                                    <p className='text-[18px] text-white'>{item.text[3]}</p>
                                </div>
                            ))
                        }
                    </div>
